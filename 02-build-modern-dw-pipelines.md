...
9. Update the value of the `uniqueId` variable, setting it to the unique identifier you were assigned for this workshop. This will be the _UNIQUEID_ component of your username, as mentioned above. For example:
   ...
   ![The uniqueId variable is highlighted and its value is set to an example value of "23456".](media/ex02-notebooks-configure-set-unique-id.png "Synapse Analytics Notebooks")
...
5. In the configuration panel of the cloned data flow, replace the **copyX** value after the underscore in the Name with your assigned _UNIQUEID_ from your username. For example, `EnrichCustomerData_A03`.
...
13. On the **Settings** tab, select the **Data flow** drop-down list and select your cloned data flow from the list.
    ...
    > While you are on the Settings tab, take a moment to look at the other settings configurable on this tab. They include parameters to pass into the data flow, as well as the Integration Runtime and compute resource type and size to use. If you wish to use staging, you can also specify that here.
...
18. Let's now take a closer look at each of the activities within the pipeline. On the canvas graph, select the **Copy data** activity named `Import Customer dimension`. Below the graph is a series of tabs, each of which provides additional details about selected the activity. The **General** tab displays the name and description assigned to the activity, along with a few other properties.
...
19. Select the **Source** tab. The source defines the location from which data will be copied by the activity. The **Source dataset** field is a pointer to the location of the source data.
...
20. Next, select the **Sink** tab. The sink specifies where the copied data will be written. Similar to the Source, the sink uses a dataset to define a pointer to the target data store.
...
21. Finally, select the **Mapping** tab. On this tab, you can review and set the column mappings. As you can see on this tab, the spaces are being removed from the column names in the sink.
...
22. As you did previously, switch to the **Mapping Data Flow** activity by selecting the `Enrich Customer Data` Mapping Data Flow activity in the graph.

    > Take a minute to look at the options available on the various tabs in the configuration panel. You will noticed the properties here define how the data flow operates within the pipeline.
...
23. Now, let's take a look at the definition of the data flow the Mapping Data Flow activity references. Double-click the **Mapping Data Flow** activity on the pipeline canvas to open the underlying Data Flow in a new tab. This will open your cloned version of the data flow.
...
24. The [Data Flow canvas](https://docs.microsoft.com/azure/data-factory/concepts-data-flow-overview#data-flow-canvas) allows you to see the construction of the data flow and each component contained within it in greater detail.
...
25. To better understand how a data flow functions, let's inspect the various components. Select the `PostalCodes` data source on the data flow canvas.
...
27. The **Data preview** tab allows you to ingest a small subset of data and view it on the canvas. This functionality requires an active debug session, so for this workshop, a screenshot that displays the execution results for that tab is provided below.
...
28. Before looking at the `PostalCodeFilter`, quickly select the `+` button to the right of the `PostalCodes` data source to display a list of available transformations.
...
29. Next, select the `PostalCodeFilter` transformation in the graph on the data flow canvas.
...
30. In the **Filter settings** tab of the configuration panel, click anywhere inside the **Filter on** box.
...
31. This will open the Visual expression builder.
...
32. The filter currently applied ensures all zip codes are between 00001 and 99999. To reduce this list to only zip codes that appear in our customer data, you are going to updated the filter. Our customer data has postal codes that fall within the range of 90000 to 98000.
...
33. To update the filter and reduce the incoming zip codes to those found in our customer data, do the following:

    - Edit the `greaterOrEqual()` function by changing the `1` to `90000`.
    - Change the `99999` in the `lesserOrEqual()` function to `98000`.

34. Your updated expression should look like the code below.
...
36. Select the `DimCustomer` data source on the data flow canvas graph.
...
37. Next, select the `JoinOnPostalCode` transformation and ensure the **Join settings** tab is selected to see how you can join datasets using a simple and intuitive graphical interface.
...
38. Moving on to the next transformation, the `SelectDesiredColumns` transformation uses a **Select** schema modifier to allow choosing what columns to include.
...
39. The last two items in the data flow are the defined sinks. These provide the connection settings necessary to write the transformed data into the desired data sink. Select the `EnrichCustomerData` sink and inspect the settings on the **Sink** tab.
...
40. Next, select the **Settings** tab and observe the properties set there.
...
41. Now that you've taken the time to review the data flow, let's return to your cloned copy of the pipeline. On the canvas, select the **Exercise 2 - Enrich Data_UNIQUEID** tab, where _UNIQUEID_ is the unique identifier you retrieved from your username (e.g., `A03`).
...
42. Before running the pipeline there is one more change we need to make. As mentioned above, the data flow depends on the data written by the copy activity, so you are going to add a dependency between the two activities.

43. In the data flow canvas graph, select the green box on the right-hand side of the **Copy data** activity and drag the resulting arrow up onto the **Mapping Data Flow** activity.
...
44. This creates a requirement that the **Copy data** activity completes successfully before the **Mapping Data Flow** can execute, and enforces our requirement of the Synapse Analytics table being populated before running the data flow.
...
45. The last step before running the pipeline is to publish the changes you have made. Select **Publish all** on the toolbar.
...
46. On the **Publish all** dialog, select **Publish**.

    > This Publish all dialog allows you to review the changes that will be saved. In the dialog, you should see two pending changes. The first is for your cloned pipeline and the second for your cloned data flow.
...
47. Within a few seconds, you will receive a notification that the publish completed. Select **Dismiss** in the notification.

    ![The publishing completed notification is displayed.](media/ex02-publishing-completed.png "Publishing completed")

48. Your pipeline is now ready to run. Select **Add trigger** then **Trigger now** on the toolbar for the pipeline.
...
49. Select **OK** on the Pipeline run dialog to start the pipeline run.
...
50. To monitor the pipeline run, move on to the next task.
